		...
		'schemas': {
			...
			'FinancialInstitutionIdentification': {
				...
				'properties': {
					...
					'name': {
						'type': 'string',
						'description': 'Name of the financial institution. Allowed Characters will be as per the given pattern. Characters with accent like àäáçèëéï öóòüùú and all small letters will be replaced by capital letters without accent. All other characters like (µ$£ Æ Ð × Ø Þ ß æ ÷ ø) will be blocked and payment request will be rejected.',
...
exports.up = function(knex, Promise) {
	...
	return Promise.all([
		knex('api_docs')
			.where('role', '=', 'pisp')
			.update({
				swagger: JSON.stringify(pispJsonUpdated),
				updated_at: knex.fn.now(),
			}),
		knex('api_docs')
			.where('role', '=', 'aisp')
			.update({
				swagger: JSON.stringify(aispJsonUpdated),
				updated_at: knex.fn.now(),
			}),
	])
}
...
exports.down = function(knex, Promise) {
	...
	return Promise.all([
		knex('api_docs')
			.where('role', '=', 'pisp')
			.update({
				swagger: JSON.stringify(pispJson),
				updated_at: knex.fn.now(),
			}),
		knex('api_docs')
			.where('role', '=', 'aisp')
			.update({
				swagger: JSON.stringify(aispJson),
				updated_at: knex.fn.now(),
			}),
	])
