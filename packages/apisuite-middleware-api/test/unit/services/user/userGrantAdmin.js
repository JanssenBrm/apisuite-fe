...
describe('(UNIT) services.user.userGrantAdmin', async () => {
	...
	before(async () => {
		...
		try {
			resp = await userService.userGrantAdmin(userId)
		} catch (err) {
			error = err
		}
	})
	...
	it('Should contain the admin scope', async () => {
		const hasAdminScope = resp.scopes.filter(scope => scope === 'admin').length > 0

		expect(hasAdminScope).to.be.true()
	})
})
