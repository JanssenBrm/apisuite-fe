...
LAST_SUCCESSFUL_COMMIT=`curl -Ss -u "$CIRCLE_TOKEN:" $LAST_SUCCESSFUL_BUILD_URL | jq -r 'map(select (.workflows.job_name | startswith("deploy"))) | sort_by(.author_date|select(type=="string")|split("Z")[0]|split(".")[0]+"Z"|fromdate)|[last][0]["vcs_revision"]'`
